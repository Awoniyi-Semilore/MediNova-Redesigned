...
function AudioBanner() {
  return (
    <div style={{ position:'fixed',top:0,left:0,right:0,zIndex:50, display:'flex',alignItems:'center',justifyContent:'center',gap:'0.6rem', padding:'0.55rem 1.4rem', background:'rgba(13,45,94,0.95)', backdropFilter:'blur(8px)', animation:'bannerIn 0.4s ease, bannerOut 0.4s ease 3.6s forwards' }}>
      <span style={{fontSize:'1rem',animation:'speakerPulse 1s infinite'}}>🔊</span>
      <span style={{fontFamily:"'Share Tech Mono',monospace",fontSize:'0.6rem',color:'rgba(255,255,255,0.9)',textTransform:'uppercase',letterSpacing:'0.12em'}}>Audio playing — turn up your volume</span>
    </div>
  );
}
...
function AudioIndicator({ playing }) {
  return (
    <div style={{ position:'fixed',bottom:64,right:16,zIndex:40, display:'flex',alignItems:'center',gap:5, padding:'5px 10px', background:'rgba(13,45,94,0.88)', border:'1px solid rgba(255,255,255,0.15)', borderRadius:20 }}>
      <div style={{ width:6,height:6,borderRadius:'50%', background: playing ? '#4caf50' : '#90a4ae', animation: playing ? 'audioBlink 1.2s infinite' : 'none' }}/>
      <span style={{fontFamily:"'Share Tech Mono',monospace",fontSize:'0.5rem',color:'rgba(255,255,255,0.7)',textTransform:'uppercase',letterSpacing:'0.08em'}}>{playing ? 'Playing…' : 'Audio'}</span>
    </div>
  );
}

function ecgBg(hexColor, opacity = 0.07) {
  const encoded = encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="240" height="48"><polyline points="0,24 20,24 26,10 31,38 36,6 41,28 46,24 90,24 116,24 121,10 126,38 131,6 136,28 141,24 185,24 210,24 215,10 220,38 225,6 230,28 235,24 240,24" fill="none" stroke="${hexColor}" stroke-width="0.6" opacity="${opacity}"/></svg>`);
  return { backgroundImage: `url("data:image/svg+xml,${encoded}")`, backgroundSize: '240px 48px', backgroundRepeat: 'repeat' };
}
...
function useQuestionEngine(questions, onAnswer, onSimComplete) {
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft(t => { if (t <= 1) { clearInterval(timerRef.current); reveal(null); return 0; } return t - 1; });
    }, 1000);
  ...
  function select(opt) { if (revealed) return; setSelected(opt.id); clearInterval(timerRef.current); reveal(opt); }
  function next(currentAllAnswers) { if (qIndex < questions.length - 1) setQIndex(i => i + 1); else onSimComplete(currentAllAnswers); }
  return { q, qIndex, questions, selected, revealed, timeLeft, timerPaused, shuffledOptions, allAnswers, select, next, pauseTimer, resumeTimer };
}

// DESIGN 3 — Senior Residency
export function SimDesign3SeniorRes({ cls, sim, subsim, variant, accentColor, track, onAnswer, onSimComplete }) {
  ...
  return (
    <div style={{ background:'#e8ede8', ...ecgBg('#2e7d32',0.06), minHeight:'100vh', fontFamily:"'DM Sans',sans-serif", paddingBottom:56 }}>
      ...
      <div style={{ background:'#1a2e1a', padding:'0.7rem 1.4rem', display:'flex', alignItems:'center', justifyContent:'space-between', zIndex:2, position:'relative' }}>
        <div style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'0.58rem', color:accentColor, textTransform:'uppercase', letterSpacing:'0.14em' }}>MediNova Teaching Hospital · Senior Residency · Class {cls.num}</div>
        <span style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'0.65rem', color:'#fff' }}>{new Date().toLocaleTimeString('en-GB', { hour:'2-digit', minute:'2-digit' })}</span>
      </div>
      <div style={{ padding:'1rem 1.4rem', maxWidth:860, margin:'0 auto', zIndex:2, position:'relative' }}>
        {vitals && (
          <div style={{ display:'flex', gap:8, marginBottom:'0.8rem' }}>
            {[{l:'HR', v:vitals.hr, w:vitals.hr>100}, {l:'BP', v:vitals.bp, w:false}, {l:'SpO₂', v:`${vitals.spo2}%`, w:vitals.spo2<94}, {l:'Temp', v:`${vitals.temp}°C`, w:vitals.temp>38}].map(x => (
              <div key={x.l} style={{ background:'rgba(255,255,255,0.9)', border:`1px solid ${x.w?'#c0392b':'#c0cfc0'}`, borderRadius:3, padding:'6px 10px', textAlign:'center', flex:1 }}>
                <div style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'0.7rem', color:x.w?'#c0392b':'#1a2e1a' }}>{x.v}</div>
                <div style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'0.44rem', color:'#4a6a4a' }}>{x.l}</div>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:4, marginBottom:'0.8rem' }}>
          {eng.shuffledOptions.map((opt, i) => {
            ...
            return (
              <div key={opt.id} onClick={()=>eng.select(opt)} style={{ display:'flex', gap:8, padding:'9px 11px', background:bg, border, borderRadius:3, cursor:eng.revealed?'default':'pointer', color, fontSize:'0.8rem' }}>
                <span style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'0.58rem', color:'#4a6a4a' }}>{String.fromCharCode(65+i)}</span>
        ...
        {eng.revealed && (
          <button onClick={()=>eng.next(eng.allAnswers)} style={{ width:'100%', padding:'0.8rem', background:accentColor, color:'#fff', border:'none', borderRadius:4, fontFamily:"'Share Tech Mono',monospace", fontSize:'0.62rem', cursor:'pointer' }}>{eng.qIndex < eng.questions.length-1 ? 'Next Question →' : 'Complete Simulation →'}</button>
        )}
      </div>
      <div style={{ position:'fixed', bottom:0, left:0, right:0, display:'flex', alignItems:'center', gap:'0.7rem', padding:'0.6rem 1.4rem', background:'rgba(240,245,240,0.97)', borderTop:'1px solid #c0cfc0', zIndex:10 }}>
        <div style={{ flex:1, height:3, background:'#c0cfc0', borderRadius:2, overflow:'hidden' }}><div style={{ height:'100%', width:`${timerPct}%`, background:timerColor, transition:'width 1s linear' }}/></div>
        <span style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'0.72rem', color:timerColor }}>{eng.revealed ? '—' : eng.timerPaused ? '⏸' : `0:${String(eng.timeLeft).padStart(2,'0')}`}</span>
      </div>
    </div>
  );
}
...
// DESIGN 4 — Fellowship
export function SimDesign4Fellowship({ cls, sim, subsim, variant, accentColor, track, onAnswer, onSimComplete }) {
  const questions = subsim?.questions || [];
  const eng = useQuestionEngine(questions, onAnswer, onSimComplete);
  const [showBanner, setShowBanner] = useState(false);
  const [showIndicator, setShowIndicator] = useState(false);
  ...
  return (
    <div style={{ background:dark, ...ecgBg('#9c27b0',0.05), minHeight:'100vh', fontFamily:"'DM Sans',sans-serif", paddingBottom:48 }}>
      ...
      <div style={{ background:'#120e22', borderBottom:`1px solid ${border}`, padding:'0.6rem 1rem', display:'flex', alignItems:'center', gap:'0.8rem', position:'relative', zIndex:2 }}>
        <div style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'0.6rem', color:accentColor, textTransform:'uppercase', letterSpacing:'0.14em' }}>ICU Command · Fellowship Level · Class {cls.num}</div>
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'150px 1fr', minHeight:'calc(100vh - 104px)', position:'relative', zIndex:2 }}>
        <div style={{ background:'rgba(9,7,20,0.92)', borderRight:`1px solid ${border}`, padding:'0.8rem' }}>
          {vitals && [{l:'HR', v:vitals.hr, w:vitals.hr>100}, {l:'BP', v:vitals.bp, w:false}, {l:'SpO₂', v:`${vitals.spo2}%`, w:vitals.spo2<94}].map(x => (
            <div key={x.l} style={{ background:'#120e22', border:`1px solid ${border}`, borderRadius:4, padding:'0.5rem', marginBottom:4, textAlign:'center' }}>
              ...
              <div style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'0.42rem', color:'#4a1472' }}>{x.l}</div>
            </div>
          ))}
        </div>
        <div style={{ padding:'0.9rem 1rem' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
            {eng.shuffledOptions.map((opt, i) => {
              ...
              return (
                <div key={opt.id} onClick={()=>eng.select(opt)} style={{ display:'flex', gap:8, padding:'8px 10px', background:bg, border:bord, borderRadius:4, cursor:eng.revealed?'default':'pointer', fontSize:'0.75rem', color }}>
                  <span style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'0.58rem' }}>{String.fromCharCode(65+i)}</span>
                  <span style={{flex:1}}>{opt.text}</span>
                </div>
              );
            })}
          </div>
          {eng.revealed && <button onClick={()=>eng.next(eng.allAnswers)} style={{ marginTop:'1rem', padding:'0.75rem 1.4rem', background:accentColor, color:'#fff', border:'none', borderRadius:6, fontFamily:"'Share Tech Mono',monospace", fontSize:'0.62rem', cursor:'pointer' }}>{eng.qIndex < eng.questions.length-1 ? 'Next →' : 'Complete →'}</button>}
        </div>
      </div>
      <div style={{ position:'fixed', bottom:0, left:0, right:0, display:'flex', alignItems:'center', gap:'0.6rem', padding:'0.5rem 1rem', background:'rgba(8,6,18,0.97)', borderTop:`1px solid #1a1030`, zIndex:10 }}>
        <div style={{ flex:1, height:3, background:border, borderRadius:2 }}><div style={{ height:'100%', width:`${timerPct}%`, background:timerColor, transition:'width 1s linear' }}/></div>
        <span style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'0.72rem', color:timerColor }}>{eng.revealed ? '—' : eng.timerPaused ? '⏸' : `0:${String(eng.timeLeft).padStart(2,'0')}`}</span>
      </div>
    </div>
  );
...
// DESIGN 5 — Board Certification
export function SimDesign5Board({ cls, sim, subsim, variant, accentColor, track, onAnswer, onSimComplete }) {
  const questions = subsim?.questions || [];
  const eng = useQuestionEngine(questions, onAnswer, onSimComplete);
  const [showBanner, setShowBanner] = useState(false);
  const [showIndicator, setShowIndicator] = useState(false);
  ...
  return (
    <div style={{ background:'#0c0a08', ...ecgBg('#8b6914',0.07), minHeight:'100vh', fontFamily:"'DM Sans',sans-serif", display:'flex', flexDirection:'column' }}>
      ...
      {showBanner && <AudioBanner />}
      {showIndicator && <AudioIndicator playing={audioPlaying} />}
      <div style={{ background:'#120e0a', borderBottom:'1px solid #2a1f14', padding:'0.7rem 1.4rem', display:'flex', alignItems:'center', justifyContent:'space-between', zIndex:2, position:'relative' }}>
        <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'0.9rem', color:gold, fontWeight:600 }}>MediNova Board of Examiners</div>
      ...
      <div style={{ flex:1, padding:'1.4rem', maxWidth:760, margin:'0 auto', width:'100%', zIndex:2, position:'relative' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:5, marginBottom:'1rem' }}>
          {eng.shuffledOptions.map((opt, i) => {
            ...
            return (
              <div key={opt.id} onClick={()=>eng.select(opt)} style={{ display:'flex', gap:10, padding:'0.75rem 1rem', background:bg, border:bord, borderRadius:2, cursor:eng.revealed?'default':'pointer', color }}>
                <span style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'0.6rem', color:'#3d2a14' }}>{String.fromCharCode(65+i)}</span>
                <span style={{ fontSize:'0.82rem', flex:1 }}>{opt.text}</span>
              </div>
            );
          })}
        </div>
        {eng.revealed && <button onClick={()=>eng.next(eng.allAnswers)} style={{ padding:'0.85rem 2rem', background:gold, color:'#0c0a08', border:'none', borderRadius:2, fontFamily:"'Share Tech Mono',monospace", fontSize:'0.65rem', cursor:'pointer', fontWeight:600 }}>{eng.qIndex < eng.questions.length-1 ? 'Next Record →' : 'Submit to Board →'}</button>}
      </div>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0.7rem 1.4rem', background:'rgba(8,6,4,0.97)', borderTop:'1px solid #2a1f14', zIndex:2, position:'relative' }}>
        <div style={{ display:'flex', gap:3 }}>{Array.from({length:totalQuestions}).map((_, i) => <div key={i} style={{ width:7, height:7, borderRadius:'50%', background: i < eng.qIndex ? gold : i === eng.qIndex ? '#d4a843' : '#2a1f14' }}/>)}</div>
      </div>
    </div>
  );
}