    ...
    <div style={{ background:'#e8ede8', ...ecgBg('#2e7d32',0.06), minHeight:'100vh', fontFamily:"'DM Sans',sans-serif", paddingBottom:56 }}>
      ...
      <div style={{ background:'#1a2e1a', padding:'0.7rem 1.4rem', display:'flex', alignItems:'center', justifyContent:'space-between', zIndex:2, position:'relative' }}>
        <div style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'0.58rem', color:accentColor, textTransform:'uppercase', letterSpacing:'0.14em' }}>
          MediNova Teaching Hospital · Senior Residency · Class {cls.num}
        </div>
      ...
      <div style={{ padding:'1rem 1.4rem', maxWidth:860, margin:'0 auto', zIndex:2, position:'relative' }}>
        ...
        {hasAudio && (
          <div style={{ display:'flex', alignItems:'center', gap:'0.7rem', background:'rgba(255,255,255,0.8)', border:`1px solid ${accentColor}44`, borderRadius:4, padding:'0.5rem 0.8rem', marginBottom:'0.8rem' }}>
            ...
            <button onClick={()=>audioPlaying ? audioRef.current?.pause() : audioRef.current?.play()} style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'0.55rem', padding:'4px 10px', background:'transparent', border:`1px solid ${accentColor}`, borderRadius:3, cursor:'pointer', color:accentColor }}>
              {audioPlaying ? '■ Pause' : '▶ Play Audio'}
            </button>
          </div>
        )}

        <div style={{ fontSize:'0.85rem', color:'#1a2e1a', fontWeight:500, padding:'0.7rem 0.9rem', background:'rgba(255,255,255,0.9)', borderLeft:`3px solid #c0392b`, borderRadius:'0 4px 4px 0', marginBottom:'0.8rem' }}>
          {eng.q.stem}
        </div>
        ...
        {eng.revealed && (
          <button onClick={()=>eng.next(eng.allAnswers)} style={{ width:'100%', padding:'0.8rem', background:accentColor, color:'#fff', border:'none', borderRadius:4, fontFamily:"'Share Tech Mono',monospace", fontSize:'0.62rem', cursor:'pointer' }}>
      ...
      <div style={{ position:'fixed', bottom:0, left:0, right:0, display:'flex', alignItems:'center', gap:'0.7rem', padding:'0.6rem 1.4rem', background:'rgba(240,245,240,0.97)', borderTop:'1px solid #c0cfc0', zIndex:10 }}>
        <div style={{ flex:1, height:3, background:'#c0cfc0', borderRadius:2, overflow:'hidden' }}>
    ...
    <div style={{ background:dark, ...ecgBg('#9c27b0',0.05), minHeight:'100vh', fontFamily:"'DM Sans',sans-serif", paddingBottom:48 }}>
      ...
      <div style={{ background:'#120e22', borderBottom:`1px solid ${border}`, padding:'0.6rem 1rem', display:'flex', alignItems:'center', gap:'0.8rem', position:'relative', zIndex:2 }}>
        <div style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'0.6rem', color:accentColor, textTransform:'uppercase', letterSpacing:'0.14em' }}>
          ICU Command · Fellowship Level · Class {cls.num}
        </div>
      </div>
        ...
        <div style={{ padding:'0.9rem 1rem' }}>
          ...
          {hasAudio && (
            <div style={{ display:'flex', alignItems:'center', gap:'0.7rem', background:`${accentColor}0d`, border:`1px solid ${accentColor}33`, borderRadius:4, padding:'0.5rem 0.8rem', marginBottom:'0.7rem' }}>
              <audio ref={audioRef} src={audioSrc} onPlay={()=>setAudioPlaying(true)} onPause={()=>setAudioPlaying(false)} onEnded={()=>setAudioPlaying(false)} />
              <button onClick={()=>audioPlaying ? audioRef.current?.pause() : audioRef.current?.play()} style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'0.55rem', background:'transparent', border:`1px solid ${accentColor}`, borderRadius:3, cursor:'pointer', color:accentColor }}>
                {audioPlaying ? 'Pause' : 'Play Audio'}
              </button>
            </div>
          )}
          ...
          {eng.revealed && (
            <button onClick={()=>eng.next(eng.allAnswers)} style={{ marginTop:'1rem', padding:'0.75rem 1.4rem', background:accentColor, color:'#fff', border:'none', borderRadius:6, fontFamily:"'Share Tech Mono',monospace", fontSize:'0.62rem', cursor:'pointer' }}>
  ...
  return (
    <div style={{ background:'#0c0a08', ...ecgBg('#8b6914',0.07), minHeight:'100vh', fontFamily:"'DM Sans',sans-serif", display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ flex:1, padding:'1.4rem', maxWidth:760, margin:'0 auto', width:'100%', zIndex:2, position:'relative' }}>
        <div style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'0.55rem', color:'#3d2a14', textTransform:'uppercase', marginBottom:'0.7rem' }}>
          Record #{eng.qIndex+1} of {totalQuestions} · Clinical Assessment
        </div>
        ...
        {hasAudio && (
          <div style={{ display:'flex', alignItems:'center', gap:'0.7rem', background:`${gold}0d`, border:`1px solid ${gold}33`, borderRadius:4, padding:'0.5rem 0.8rem', marginBottom:'0.8rem' }}>
            <audio ref={audioRef} src={audioSrc} onPlay={()=>setAudioPlaying(true)} onPause={()=>setAudioPlaying(false)} onEnded={()=>setAudioPlaying(false)} />
            <button onClick={()=>audioPlaying ? audioRef.current?.pause() : audioRef.current?.play()} style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:'0.55rem', background:'transparent', border:`1px solid ${gold}`, cursor:'pointer', color:gold }}>
        ...
        {eng.revealed && (
          <button onClick={()=>eng.next(eng.allAnswers)} style={{ padding:'0.85rem 2rem', background:gold, color:'#0c0a08', border:'none', borderRadius:2, fontFamily:"'Share Tech Mono',monospace", fontSize:'0.65rem', cursor:'pointer', fontWeight:600 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0.7rem 1.4rem', background:'rgba(8,6,4,0.97)', borderTop:'1px solid #2a1f14', zIndex:2, position:'relative' }}>
        <div style={{ display:'flex', gap:3 }}>
          {Array.from({length:totalQuestions}).map((_, i) => (
            <div key={i} style={{ width:7, height:7, borderRadius:'50%', background: i < eng.qIndex ? gold : i === eng.qIndex ? '#d4a843' : '#2a1f14' }}/>
          ))}
        </div>
      </div>
    </div>
